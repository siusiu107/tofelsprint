...
*/
(function(){
  // 캐시가 꼬이면 window.TOEFL이 undefined가 되면서 시뮬 화면이 하얗게 뜰 수 있어.
  // 1회 자동으로 캐시 버스터 리로드를 시도한다.
  const TOEFL = window.TOEFL;
  if(!TOEFL){
    try{
      const k = 'toefl_boot_reload_once_v2';
  ...
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#ffffff">
    <div style="width:min(520px,92vw);padding:22px 20px;font-family:system-ui;line-height:1.6;text-align:center;border:1px solid #eee;border-radius:18px;box-shadow:0 8px 22px rgba(0,0,0,.06)">
      <div style="font-weight:900;font-size:20px">로딩 중…</div>
      <div style="color:#666;margin-top:10px">
        잠깐만 기다려줘. 네트워크/캐시 상태에 따라 처음 1회 로딩이 꼬일 수 있어.
      </div>
      <button id="bootRetryBtn" style="margin-top:16px;padding:10px 14px;border-radius:12px;border:none;background:#7f9a54;color:#fff;font-weight:900;cursor:pointer">
        다시 시도
      </button>
    </div>
  </div>`;
try{
  const b = document.getElementById('bootRetryBtn');
  if(b) b.onclick = ()=>{
    try{
      const u = new URL(location.href);
      u.searchParams.set('_r', String(Date.now()));
  ...
  function renderPassageHtml(text){
    ...
    return (text||'')
      .split(/\n\s*\n/g)
      .map(p=>`<p class="p">${escape(p).replace(/\n/g,' ')}</p>`)
      .join('');
  }
        ...
        <section class="panel">
          <div class="panel-head">
            <div>
              ...
              <div class="muted small">${escape(it.title||data.title||'')}</div>
            </div>
            <div class="row">
              <span class="badge">Passage ${pNum}/${readingPlan.length}</span>
            </div>
          </div>
          <div class="panel-body" id="simPassage">${renderPassageHtml(data.passage)}</div>
        </section>
    ...
    function highlightSentenceFromPrompt(prompt){
      ...
      const tryFind = (h)=>{
        ...
        for(const pEl of paras){
          ...
          const m = re.exec(raw);
          if(m) return { pEl, raw, idx: m.index, len: m[0].length };
        }
        return null;
      };
      ...
      // 스크롤: 하이라이트(문장) 자체가 화면 중간 근처에 오도록
      try{
