...
function renderHome(){
  ...
  async function renderReadingViewer(id){
    ...
    function highlightSentenceFromPrompt(prompt){
      ...
      // 스크롤이 너무 아래로 떨어지지 않게: 하이라이트 문장을 패널 중앙쯤에 오게 맞춤
      try{
        const targetTop = pEl.offsetTop - (passageEl.clientHeight * 0.45) + (pEl.offsetHeight * 0.5);
        passageEl.scrollTo({ top: Math.max(0, targetTop), behavior: 'smooth' });
      }catch(e){}
    }
